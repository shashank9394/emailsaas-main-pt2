...
export async function generateEmail(context: string, prompt: string) {
    ...
    (async () => {
        try {
            ...
            stream.update("Sorry, there was an error generating your email. Please try again.");
            stream.done();
        }
    })();
...
export async function generate(input: string) {
    ...
    (async () => {
        try {
            ...
            // Use a more powerful model for the text completion feature if needed
            const completionModel = genAI.getGenerativeModel({ model: "gemini-1.5-pro" });
            
            // Generate content with streaming
            const result = await completionModel.generateContentStream(geminiPrompt);
            ...
            stream.update("Error completing text.");
            stream.done();
        }
    })();
